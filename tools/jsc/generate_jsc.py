...
if __name__ == '__main__':
    options = {
        ...
        'compressed_filename':'game.min.js', 
    ...
    run = CCPluginJSCompile()
    run.init(options, workingdir)

    # create output directory
    try:
        os.makedirs(run._dst_dir)
    except OSError:
        if os.path.exists(run._dst_dir) == False:
            raise Exception("Error: cannot create folder in "+ run._dst_dir)

    # deep iterate the src directory
    for src_dir in run._src_dir_arr:
        run._current_src_dir = src_dir
        run._js_files[run._current_src_dir] = []
        run.deep_iterate_dir(src_dir)

    run.reorder_js_files()
    run.handle_all_js_files()
    print "\nCompilation finished, (%d) files succeed, (%d) files fail." % (len(run._success), len(run._failure))
    if len(run._failure) > 0:
